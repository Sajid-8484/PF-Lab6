...
int main() {
    ...
    char choice;
    int seat[100] = {0}; // simple array to remember reserved seats (0 = free, 1 = reserved)
    ...
    do {
        ...
        for (int i = 1; i <= totalSeats; i++) {
            if (seat[i] == 0) // only show seats that are not reserved
                cout << "[" << i << "] ";
        }
        ...
        if (reservedSeat < 1 || reservedSeat > totalSeats) {
            ...
            seat[reservedSeat] = 1; // mark as reserved
